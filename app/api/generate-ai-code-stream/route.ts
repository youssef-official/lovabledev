...
declare global {
  ...
  var conversationStates: Record<string, ConversationState>;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prompt, model = 'openai/gpt-oss-20b', context, isEdit = false, projectId } = body;
    ...
    // Initialize conversation state for the specific project if not exists
    if (!global.conversationStates[projectId]) {
      global.conversationStates[projectId] = {
    ...
    const conversationState = global.conversationStates[projectId];
    ...
    if (conversationState.context.messages.length > 20) {
      ...
      console.log('[generate-ai-code-stream] Trimmed conversation history for project', projectId);
    }
